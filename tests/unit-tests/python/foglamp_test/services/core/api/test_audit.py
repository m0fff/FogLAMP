...
storage_client = StorageClient("0.0.0.0", core_management_port=38215)
...
class TestAudit:
    ...
    async def test_get_audit_with_invalid_severity(self):
        ...
        r = conn.getresponse()
        assert 400 == r.status
        assert "'BLA' not a valid severity" == r.reason
    ...
    async def test_get_audit_limit_with_bad_data(self):
        conn = http.client.HTTPConnection(BASE_URL)
        conn.request("GET", '/foglamp/audit?limit=blah')
        r = conn.getresponse()
        assert 400 == r.status
        assert "Limit must be an integer" == r.reason

    async def test_get_audit_skip_with_bad_data(self):
        conn = http.client.HTTPConnection(BASE_URL)
        conn.request("GET", '/foglamp/audit?skip=blah')
        r = conn.getresponse()
        assert 400 == r.status
        assert "Skip/Offset must be an integer" == r.reason
